...
const lk = new ToolKit(`一汽丰田签到`, `yqft_token`)
const yqftTokenKey = 'yqft_token'
let yqftToken = lk.getVal(yqftTokenKey, '')
lk.userAgent = "ToyotaUccn/5.6.2 (com.frontnetwork.FTMS; build:202303151747; iOS 16.4.1) Alamofire/5.4.4"
...
function getCookie() {
    if (lk.isGetCookie(/\/wt\/getUserInfo/)) {
        lk.log(`开始获取cookie`)
        let data = $request.headers['token']
...
async function all() {
    ...
    if (yqftToken == '') {
        lk.execFail()
        lk.appendNotifyInfo(`⚠️请先打开一汽丰田登录获取refresh_token`)
    } else {
        await refreshToken()
